...
By default attachments land on your Desktop, in:

    jira_attachments/<ISSUE-KEY>/<filename>

Set OUTPUT_DIR in the CONFIG block to put them somewhere else.
"""
...
#   None    = a "jira_attachments" folder on your Desktop, found automatically
#             on both macOS and Windows (OneDrive-redirected Desktops included)
#   "path"  = any path you like; relative paths resolve against the current
#             directory, e.g. "/Users/ianevans/Desktop/jira_stuff"
OUTPUT_DIR = None

# Folder created on the Desktop when OUTPUT_DIR is None.
DESKTOP_FOLDER_NAME = "jira_attachments"
...
def desktop_dir():
    """Locate the user's Desktop on macOS or Windows.

    Windows can redirect the Desktop into OneDrive, in which case
    %USERPROFILE%\\Desktop does not exist -- so ask the registry where the
    Desktop actually is rather than assuming, and fall back if that fails.
    """
    ...
    if os.name == "nt":
        try:
            ...
            with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path) as key:
                resolved = os.path.expandvars(winreg.QueryValueEx(key, "Desktop")[0])
            if resolved and os.path.isdir(resolved):
                return resolved
        except OSError:
            pass  # fall through to the guesses below
...
def resolve_output_root():
    """Turn the OUTPUT_DIR setting into an absolute path."""
    if OUTPUT_DIR:
        return os.path.abspath(os.path.expanduser(OUTPUT_DIR))
    return os.path.join(desktop_dir(), DESKTOP_FOLDER_NAME)
...
def check_config(output_root):
    ...
    if os.name == "nt":
        ...
        worst = len(output_root) + 1 + 24 + 1 + MAX_FILENAME_CHARS
        if worst > 255:
            log(
